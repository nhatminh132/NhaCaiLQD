...
class BettingBot(discord.Client):
    ...
    # --- VÒNG LẶP TỰ ĐỘNG TÌM KÈO ---
    @tasks.loop(hours=6)
    async def autofind_task(self):
    ...
    @autofind_task.before_loop
    async def before_autofind_task(self):
        # Đổi tần suất chạy dựa trên cài đặt
        frequency_hours = get_int_setting('autofind_frequency', 6)
        self.autofind_task.change_interval(hours=frequency_hours)
        print(f"Tần suất tự động tìm kèo được đặt là: {frequency_hours} giờ.")
        await self.wait_until_ready()

    # --- VÒNG LẶP TỰ ĐỘNG CẬP NHẬT ROLE ---
    ...
    async def update_roles_task(self):
        ...
        # Lấy tất cả các guilds mà bot đang ở
        for guild in self.guilds:
            cursor.execute("SELECT role_id, min_tokens FROM role_tiers WHERE guild_id = ? ORDER BY min_tokens DESC", (guild.id,))
            tiers = cursor.fetchall()
            if not tiers: continue # Server này không cài đặt role tiers
            
            # Tạo một map các role_id để dễ truy cập
            tier_roles = {}
            ...
            for role_id, min_tokens in tiers:
                ...
                if role:
                    ...
                    print(f"Cảnh báo: Không tìm thấy Role ID {role_id} ở Guild {guild.name}. Cân nhắc xóa nó.")
            ...
            for user_id, balance in all_users:
                try:
                    member = guild.get_member(user_id) # Lấy member object
                    if not member: continue # User không ở trong server này
                    
                    # Xác định role user nên có
                    correct_role = None
                    for min_tokens, role in tier_roles.items():
                        if balance >= min_tokens:
                            correct_role = role
                            break # Vì đã sắp xếp DESC
                    
                    if not correct_role: # User có 0 token và không có role 0-token
                        continue
                    ...
                    print(f"Lỗi: Không có quyền (Forbidden) để quản lý role cho {user_id} ở {guild.name}. Hãy đảm bảo role của Bot ở trên các role tier.")
...
async def internal_create_match(interaction: discord.Interaction, internal_id: str, team_a: str, team_b: str, api_id: int):
    ...
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    try:
        cursor.execute("INSERT INTO matches (match_id, team_a, team_b, api_match_id, status) VALUES (?, ?, ?, ?, 'open')", (internal_id, team_a, team_b, api_id))
        conn.commit()
...
async def internal_resolve_logic(interaction_response_method, match_id: str, winner_team_name: str):
    """Hàm lõi xử lý trả thưởng (Nâng cấp hỗ trợ /stats và /challenge)."""
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    ...
    if total_winning_pot == 0:
        ...
        for user_id, amount in winning_bets:
            update_balance(user_id, amount) # Hoàn tiền
            user = await client.fetch_user(user_id); response_msg += f"- {user.mention} nhận lại {amount} token.\n"
            # Hoàn tiền không tính là thắng hay thua, P/L = 0
    else:
        ...
        for user_id, amount in winning_bets:
            ...
            update_stats(user_id, is_win=True, profit_loss=profit)
            user = await client.fetch_user(user_id)
            response_msg += f"- {user.mention} (cược {amount}) nhận **{payout} token** (lời {profit}!).\n"
        response_msg += "\n--- Phe thua ---\n"
        for user_id, amount in (losing_bets_1 + losing_bets_2):
            update_stats(user_id, is_win=False, profit_loss=-amount)
    ...
    if not challenges:
        response_msg += "Không có thách đấu 1v1 nào cho trận này.\n"
    else:
        for chal in challenges:
            ...
            if winning_pool_name == p1_bet:
                winner_id, loser_id = p1_id, p2_id
                winner_name, loser_name = p1.mention, p2.mention
            elif winning_pool_name == p2_bet:
                winner_id, loser_id = p2_id, p1_id
                winner_name, loser_name = p2.mention, p1.mention
            
            if winner_id:
                # Trả thưởng (tiền đã bị trừ lúc accept)
                payout = amount * 2
                update_balance(winner_id, payout)
                update_stats(winner_id, is_win=True, profit_loss=amount)
                update_stats(loser_id, is_win=False, profit_loss=-amount)
                response_msg += f"🔥 {winner_name} thắng {loser_name} và nhận **{payout} token**!\n"
            else:
                # HÒA (cả 2 cược A/B nhưng kết quả HÒA)
                update_balance(p1_id, amount) # Hoàn tiền
                update_balance(p2_id, amount) # Hoàn tiền
    ...
    conn.commit() # Commit tất cả thay đổi (stats, balance, challenges)
    conn.close()
    await interaction_response_method(response_msg)

# --- 5. CÁC LỚP UI (Nâng cấp) ---
...
class SettingsModal(ui.Modal, title='Cài đặt chung cho Bot'):
    ...
    def __init__(self):
        ...
        self.add_item(self.autofind_frequency)
        
    async def on_submit(self, interaction: discord.Interaction):
        try:
            daily = int(self.daily_amount.value)
            starting = int(self.starting_balance.value)
            frequency = int(self.autofind_frequency.value)
            
            set_setting('daily_amount', daily)
            set_setting('starting_balance', starting)
            set_setting('autofind_frequency', frequency)
            
            # Cập nhật lại tần suất task
            client.autofind_task.change_interval(hours=frequency)
            
            await interaction.response.send_message("✅ Cài đặt đã được cập nhật!\n"
                                                  f"- /daily: {daily} token\n"
                                                  f"- /register: {starting} token\n"
                                                  f"- Tần suất AutoFind: {frequency} giờ (sẽ áp dụng ở lần chạy sau).", ephemeral=True)
        except ValueError:
            await interaction.response.send_message("Lỗi: Vui lòng chỉ nhập số nguyên.", ephemeral=True)

class ChallengeView(ui.View):
    """View chứa nút Chấp nhận/Từ chối thách đấu."""
    def __init__(self):
    ...
    async def accept(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if not challenge:
            await interaction.response.send_message("Kèo thách đấu này đã hết hạn hoặc đã được xử lý.", ephemeral=True); conn.close(); return
        
        # challenge_id, match_id, challenger_id, opponent_id, ...
        opponent_id, amount = challenge[3], challenge[6]
        
        if interaction.user.id != opponent_id:
            await interaction.response.send_message("Bạn không phải là người được thách đấu!", ephemeral=True); conn.close(); return

        # Kiểm tra tiền
        opponent_balance = get_balance(opponent_id)
        if opponent_balance < amount:
            await interaction.response.send_message(f"Bạn không đủ {amount} token để chấp nhận kèo này.", ephemeral=True); conn.close(); return
            
        challenger_id = challenge[2]
        challenger_balance = get_balance(challenger_id)
        if challenger_balance < amount:
            # Người thách đấu hết tiền
            ...
            await interaction.response.send_message("Kèo đã bị hủy do người thách đấu không còn đủ tiền.", ephemeral=True)
            return

        # Trừ tiền cả 2
        update_balance(challenger_id, -amount)
        update_balance(opponent_id, -amount)
        
        # Cập nhật DB
        cursor.execute("UPDATE challenges SET status = 'accepted' WHERE challenge_id = ?", (challenge[0],)); conn.commit(); conn.close()
        
        button.disabled = True
        self.children[1].disabled = True # Tắt nút Từ chối
        await interaction.message.edit(view=self)
        await interaction.response.send_message(f"✅ {interaction.user.mention} đã chấp nhận kèo! Cả hai đã bị trừ {amount} token. Chúc may mắn!")
    ...
    async def decline(self, interaction: discord.Interaction, button: ui.Button):
        ...
        button.disabled = True
        self.children[0].disabled = True # Tắt nút Chấp nhận
...
def get_upcoming_fixtures_from_watchlist(days_ahead=3):
    """Hàm API mới cho lệnh /upcoming."""
    if not RAPIDAPI_KEY: return []
    
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    cursor.execute("SELECT league_id, league_name FROM watched_leagues")
    leagues = cursor.fetchall(); conn.close()
    ...
    for league_id, league_name in leagues:
        querystring = {
            ...
            "from": date_from,
            "to": date_to,
            "status": "NS" # Chỉ lấy các trận chưa đá (Not Started)
        }
        try:
            response = requests.get(url, headers=headers, params=querystring, timeout=10)
            response.raise_for_status(); data = response.json()
            ...
            for fixture in data['response']:
                fixture['league_name'] = league_name # Thêm tên giải để dễ hiển thị
                all_fixtures.append(fixture)
        except Exception as e:
            print(f"Lỗi khi lấy /upcoming cho giải {league_id}: {e}")

    # Sắp xếp các trận theo ngày giờ
...
async def balance(interaction: discord.Interaction):
    ...
    if user_balance is None:
        await interaction.response.send_message('Bạn chưa đăng ký! Dùng lệnh `/register` trước nhé.', ephemeral=True)
    else:
        await interaction.response.send_message(f'💰 Số token của bạn là: **{user_balance}** token.', ephemeral=True)
...
async def bet(interaction: discord.Interaction, id: str, đội: str, tiền: int):
    ...
    if đội_lower == team_a.lower(): team_bet_on = team_a.lower()
        ...
        await interaction.response.send_message(f"Tên cược không hợp lệ. Vui lòng chọn `{team_a}`, `{team_b}`, hoặc `HÒA`.", ephemeral=True)
        conn.close(); return
...
async def daily(interaction: discord.Interaction):
    ...
    last_daily_str, balance = result
    last_daily_dt = datetime.fromisoformat(last_daily_str)
    
    if datetime.now() - last_daily_dt < timedelta(hours=24):
        cooldown_ends = last_daily_dt + timedelta(hours=24)
        await interaction.response.send_message(f'Bạn đã nhận thưởng hôm nay rồi. Vui lòng quay lại sau <t:{int(cooldown_ends.timestamp())}:R>.', ephemeral=True)
    else:
        new_balance = balance + daily_amount
        cursor.execute("UPDATE users SET balance = ?, last_daily = ? WHERE user_id = ?", (new_balance, datetime.now().isoformat(), user_id))
        conn.commit()
        await interaction.response.send_message(f'🎉 Bạn đã nhận được **{daily_amount} token**! Số dư mới của bạn là {new_balance} token.')
    conn.close()
...
async def transfer(interaction: discord.Interaction, người_nhận: discord.Member, số_tiền: int):
    ...
    if sender_id == receiver_id: await interaction.response.send_message("Bạn không thể tự chuyển token cho chính mình.", ephemeral=True); return
    ...
    if get_balance(receiver_id) is None: 
        starting_balance = get_int_setting('starting_balance', 100)
        register_user(receiver_id, starting_balance)
...
async def leaderboard(interaction: discord.Interaction):
    ...
    for i, (user_id, balance) in enumerate(results):
        ...
        except discord.NotFound: user_name = f"Người dùng (ID: {user_id})"
...
async def match_info(interaction: discord.Interaction, id: str):
    ...
    team_a, team_b, status = match
    cursor.execute("SELECT team_bet_on, SUM(amount), COUNT(user_id) FROM bets WHERE match_id = ? GROUP BY team_bet_on", (id,))
    pools = cursor.fetchall(); conn.close()
...
async def stats(interaction: discord.Interaction):
    user_id = interaction.user.id
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    cursor.execute("SELECT balance, wins, losses, profit_loss FROM users WHERE user_id = ?", (user_id,))
    result = cursor.fetchone()
    conn.close()
    
    if result is None:
        await interaction.response.send_message("Bạn chưa đăng ký! Dùng `/register`.", ephemeral=True); return
...
async def upcoming(interaction: discord.Interaction):
    ...
    if isinstance(fixtures_data, str): # Trả về "Watchlist trống."
        await interaction.followup.send(fixtures_data, ephemeral=True); return
    if not fixtures_data:
        await interaction.followup.send("Không tìm thấy trận đấu nào (chưa bắt đầu) trong 3 ngày tới thuộc watchlist của server."); return
        
    embed = discord.Embed(title="📅 Các kèo sắp diễn ra (3 ngày tới)", color=discord.Color.teal())
    
    # Gom nhóm theo giải đấu
    leagues = {}
    ...
    for league_name, fixtures in leagues.items():
        field_value = ""
        for f in fixtures:
            team_a, team_b = f['teams']['home']['name'], f['teams']['away']['name']
            timestamp = f['fixture']['timestamp']
            field_value += f"- {team_a} vs {team_b} (<t:{timestamp}:R>)\n"
        embed.add_field(name=f"⚽ {league_name}", value=field_value, inline=False)
        
    await interaction.followup.send(embed=embed)

# --- (MỚI) Lệnh User /challenge ---
@client.tree.command(name="challenge", description="Thách đấu 1v1 với một người dùng khác.")
@app_commands.describe(user="Người bạn muốn thách đấu", 
                        match_id="ID nội bộ của trận đấu", 
                        cược_cho="Đội bạn cược (không thể cược HÒA)", 
                        số_tiền="Số token thách đấu")
async def challenge(interaction: discord.Interaction, user: discord.Member, match_id: str, cược_cho: str, số_tiền: int):
    ...
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    cursor.execute("SELECT team_a, team_b, status FROM matches WHERE match_id = ?", (match_id,))
    match = cursor.fetchone()
    ...
    if cược_cho.lower() == team_a.lower():
        challenger_bet_on, opponent_bet_on = team_a.lower(), team_b.lower()
    elif cược_cho.lower() == team_b.lower():
        challenger_bet_on, opponent_bet_on = team_b.lower(), team_a.lower()
    else:
        await interaction.response.send_message("Thách đấu 1v1 chỉ hỗ trợ cược cho Đội A hoặc Đội B (không cược HÒA).", ephemeral=True); conn.close(); return

    try:
        cursor.execute("""
        INSERT INTO challenges (match_id, challenger_id, opponent_id, challenger_bet_on, opponent_bet_on, amount, status)
        VALUES (?, ?, ?, ?, ?, ?, 'pending')
        """, (match_id, challenger_id, opponent_id, challenger_bet_on, opponent_bet_on, số_tiền))
        challenge_id = cursor.lastrowid
        conn.commit()
        
        embed = discord.Embed(title="⚔️ THÁCH ĐẤU 1V1 ⚔️", color=discord.Color.red())
        embed.description = f"{interaction.user.mention} thách đấu {user.mention}!"
        embed.add_field(name="Trận đấu", value=f"`{match_id}` ({team_a} vs {team_b})")
        embed.add_field(name="Tiền cược", value=f"**{số_tiền} token**")
        ...
        # Lưu message_id để các nút bấm có thể tìm lại
        cursor.execute("UPDATE challenges SET message_id = ? WHERE challenge_id = ?", (message.id, challenge_id))
        conn.commit()
...
async def cancel_match(interaction: discord.Interaction, id: str):
    ...
    for user_id, amount in bets:
        update_balance(user_id, amount); user = await client.fetch_user(user_id); refund_msg += f"- Pool: {user.mention} nhận lại {amount} token.\n"
    ...
    cursor.execute("UPDATE challenges SET status = 'cancelled' WHERE match_id = ? AND status = 'accepted'", (id,))
    conn.commit(); conn.close()
...
async def edit_match(interaction: discord.Interaction, id: str, team_a: str = None, team_b: str = None, api_id: int = 0):
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    cursor.execute("SELECT team_a, team_b, api_match_id FROM matches WHERE match_id = ?", (id,))
    result = cursor.fetchone()
    if result is None: await interaction.response.send_message(f'Lỗi: Trận đấu `{id}` không tồn tại.', ephemeral=True); conn.close(); return
    ...
    cursor.execute("UPDATE matches SET team_a = ?, team_b = ?, api_match_id = ? WHERE match_id = ?", (new_a, new_b, new_api_id, id))
    conn.commit(); conn.close()
    await interaction.response.send_message(f'✅ Kèo `{id}` đã cập nhật.', ephemeral=True)

# --- (MỚI) Lệnh Admin /admin_adjust_tokens ---
...
async def admin_adjust_tokens(interaction: discord.Interaction, user: discord.Member, amount: int, reason: str = None):
    ...
    if user_balance is None:
        starting_balance = get_int_setting('starting_balance', 100)
        register_user(user.id, starting_balance)
        
    update_balance(user.id, amount)
    new_balance = get_balance(user.id)
    
    action_str = "cộng" if amount > 0 else "trừ"
    reason_str = f" với lý do: {reason}" if reason else ""
    
    await interaction.response.send_message(f"✅ Đã {action_str} **{abs(amount)} token** cho {user.mention}{reason_str}.\n"
                                            f"Số dư mới của họ là: **{new_balance} token**.")

# --- (MỚI) Lệnh Admin /settings ---
...
async def set_role_tier(interaction: discord.Interaction, role: discord.Role, min_tokens: int):
    ...
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    try:
        cursor.execute("INSERT OR REPLACE INTO role_tiers (role_id, min_tokens, guild_id) VALUES (?, ?, ?)",
                       (role.id, min_tokens, interaction.guild.id))
        conn.commit()
        ...
        await interaction.response.send_message(f"Lỗi: Đã có một role khác được gán cho mốc {min_tokens} token.", ephemeral=True)
    finally: conn.close()
...
async def remove_role_tier(interaction: discord.Interaction, role: discord.Role):
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    cursor.execute("DELETE FROM role_tiers WHERE role_id = ?", (role.id,))
...
async def list_role_tiers(interaction: discord.Interaction):
    if not interaction.guild: await interaction.response.send_message("Lệnh này chỉ dùng trong server.", ephemeral=True); return
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    cursor.execute("SELECT role_id, min_tokens FROM role_tiers WHERE guild_id = ? ORDER BY min_tokens DESC", (interaction.guild.id,))
    tiers = cursor.fetchall(); conn.close()
    ...
    for role_id, min_tokens in tiers:
        ...
        desc += f"{role.mention if role else f'Role (ID: {role_id})'}: **{min_tokens} token**\n"
    embed.description = desc
    await interaction.response.send_message(embed=embed, ephemeral=True)

client.tree.add_command(role_tier_group) # Thêm nhóm lệnh vào bot

# --- Nhóm Lệnh Watchlist (Giữ nguyên) ---
...
async def watchlist_remove(interaction: discord.Interaction, league_id: int):
    conn = sqlite3.connect('database.db'); cursor = conn.cursor()
    cursor.execute("DELETE FROM watched_leagues WHERE league_id = ?", (league_id,)); changes = conn.total_changes
    conn.commit(); conn.close()
...
# --- Nhóm Lệnh Tác vụ Nền (Giữ nguyên) ---
...
async def autofind_start(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    await interaction.response.send_message(f'✅ Đã kích hoạt tính năng tự động tìm kèo!\n'
